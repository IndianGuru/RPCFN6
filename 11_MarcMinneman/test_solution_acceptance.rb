...
class FairQueueTest < Test::Unit::TestCase
  ...
  # remaining test added by M. Minneman
  
  def test_basic6
    fd = FairDistribution.new([], 3)
    assert_equal 0, fd.time_required
    assert_distributions_are_equivalent [[], [], []], fd.distribution
  end
    
  def test_basic7
    fd = FairDistribution.new([8, 5, 3], 4)
    assert_equal 8, fd.time_required
    assert_distributions_are_equivalent [[8], [5], [3], []], fd.distribution
  end

  def test_basic8
    e = assert_raise(ArgumentError) { FairDistribution.new(8, 1) }
    assert_match(/Illegal job queue encountered/, e.message)
  end
    
  def test_basic9
    e = assert_raise(ArgumentError) { FairDistribution.new([8, 7, 6, 5, 4], [[], []]) }
    assert_match(/Illegal partition count encountered/, e.message)
  end

  def test_basic10
    fd = FairDistribution.new([8, 5, 4, 3, 2], 0)
    assert_equal 0, fd.time_required
    assert_distributions_are_equivalent [], fd.distribution
  end
